...
router.get('/textmaker', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'arthurpy') return res.json(loghandler.invalidKey)
        if (!theme) return res.json(loghandler.nottheme)
        if (theme != 'flaming' && theme != 'google-suggestion') return res.json(loghandler.notheme)
                                ...
                                .then(data => {
                                    var urlnya = data.data.url,
                                        ...
                                        res.json({
                                            status : true,
                                            Criador : `Arthur`,
                                            message : `não se esqueça de seguir Arthur`,
                                            result:{
                                                url:urlnya,
                                                delete_url: delete_url,
                                                info: 'O url irá desaparecer após 2 minutos'
                                            }
                                        })
                                })
                        })
  ...
  router.get('/textmaker1', async (req, res, next) => {
        var theme = req.query.theme,
             text = req.query.text,
             text2 = req.query.text2,
             text3 = req.query.text3,
	...
	if(apikeyInput != 'arthurpy') return res.json(loghandler.invalidKey)
        if (!theme) return res.json(loghandler.nottheme)
        if (theme != 'carved' && theme != 'google-suggestion') return res.json(loghandler.notheme)
        if (!text) return res.json(loghandler.nottext)
        ...
        if (theme == 'carved') {
            ...
            request.post({
                ...
                body: `text_1=${text}&text_2=${text2}&login=OK`,
                 ...
                 router.get('/textmaker2', async (req, res, next) => {
        var theme = req.query.theme,
             text = req.query.text,
             text2 = req.query.text2,
             text3 = req.query.text3,
	...
	if(apikeyInput != 'arthurpy') return res.json(loghandler.invalidKey)
        if (!theme) return res.json(loghandler.nottheme)
        if (theme != 'wolf' && theme != 'google-suggestion') return res.json(loghandler.notheme)
        if (!text) return res.json(loghandler.nottext)
